...
export function PortsPanel() {
  ...
  // Auto-refresh on connect and periodically
  useEffect(() => {
    ...
    <div className="h-full flex flex-col text-[13px]">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {ports.map((p) => {
          ...
          const isCreatingTunnel = forwarding === p.port;
              ...
              <span className="flex-1 text-[12px] truncate min-w-0">
                {isCreatingTunnel ? (
                  ...
                  <span
                    ...
                    onClick={() => handleOpenTunnel(p.port)}
                    title={tunnelUrl}
                  >
                    {tunnelUrl}
                  </span>
                ) : isForwarded ? (
                  <span className="text-text-muted">localhost:{p.port}</span>
                ) : (
      ...
      {/* Forwarded ports summary */}
      {forwardedPorts.size > 0 && (
        <div className="px-3 py-1.5 border-t border-border text-[11px] text-text-muted">
          {forwardedPorts.size} port{forwardedPorts.size > 1 ? "s" : ""} forwarded via devtunnel
        </div>
      )}
    </div>
  );
