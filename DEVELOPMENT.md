...
2. **Build command**: `corepack enable && pnpm install --frozen-lockfile && pnpm run build:relay:full` (root script — builds `shared` then `relay`, see [Development Scripts](#root-monorepo))
